// SUPERSEDED as the app's bundled sample: the real sample_dieline.pdf the
// brief actually references was supplied partway through this build and
// now lives at public/samples/sample_dieline.pdf, measured for real by
// scripts/measure-real-sample.ts. This script predates that — it authors a
// synthetic straight-tuck carton with a snap-lock bottom from scratch,
// reproducing the brief's headline description (154 x 95 x 16.5 mm,
// 14-ish panels) while being internally exact (perimeter identity holds
// exactly; closure residual and isometry drift both measure ~0). Kept
// around because it's still a useful, self-contained example of driving
// the full src/core pipeline (panels -> graph -> solver -> audit) as a
// correctness gate over hand-authored PDF/SVG output, and because it
// exercises the SVG extraction path the live sample button doesn't.
//
// Run with: node --experimental-strip-types scripts/generate-fixtures.ts
// (writes public/samples/sample_dieline.{pdf,svg} + sample_artwork.png —
// re-run scripts/measure-real-sample.ts afterward to restore the real
// sample as the app's bundled fixture).
...
// ---------------------------------------------------------------------------
// 1. Panel table (mm, Y-up, origin bottom-left)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 3. Raw boundary edges -> merged straight segments
// ---------------------------------------------------------------------------
...
const expectationsSource = `// AUTO-GENERATED by scripts/generate-fixtures.ts — do not edit by hand.
// Measured facts about public/samples/sample_dieline.pdf, computed by running
// this file's own straight/curve segment list through the real src/core
// pipeline (panels -> graph -> solver -> audit) at generation time.
